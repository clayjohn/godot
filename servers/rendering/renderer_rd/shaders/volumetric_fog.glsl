...
uint pack_r11_g11_b10(vec3 v) {
	uvec3 v_u = uvec3(v.r * 2047.0, v.g * 2047.0, v.b * 1023.0);
	// R and G have 11 bits each and B has 10. Then pack them into a 32 bit uint
	uint packed = v_u.r << 21 | v_u.g << 10 | v_u.b;
	return packed;
}
...
#CODE : FOG
		...
		if (abs(density) > 0.001) {
...
#ifdef EMISSION_USED
			{
				...
				emission = clamp(emission, vec3(0.0), vec3(8.0));
				// Convert to UNORM and scale values towards 0 to increase precision in low values
				emission /= vec3(8.0);
				emission = pow(emission, vec3(1.0 / 2.2));
				uvec3 emission_u = uvec3(emission.r * 2047.0, emission.g * 2047.0, emission.b * 1023.0);
				// R and G have 11 bits each and B has 10. Then pack them into a 32 bit uint
...
#ifdef ALBEDO_USED
			{
				...
				// Convert scale values towards 0 to increase precision in low values
				scattering = pow(scattering, vec3(1.0 / 2.2));
