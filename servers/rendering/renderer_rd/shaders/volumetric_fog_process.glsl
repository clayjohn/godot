...
void main() {
	...
	// Undo scaling
	scattering = pow(scattering, vec3(2.2));
	scattering += params.base_scattering * params.base_density;

	uint emissive_u = imageLoad(emissive_only_map, pos).x;
	vec3 emissive = vec3(emissive_u >> 21, (emissive_u << 11) >> 21, emissive_u % 1024) / vec3(2047.0, 2047.0, 1023.0);
	// Undo scaling and return to 0-8 range
	emissive = pow(emissive, vec3(2.2)) * 8.0;
	emissive += params.base_emissive * params.base_density;
...
*/
	...
	vec4 final_density = vec4(total_light * scattering + emissive, total_density);
